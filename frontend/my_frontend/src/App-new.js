...
function App() {
  ...
  // Check authentication and team status
  useEffect(() => {
  ...
  const showNavbar = user && !['/', '/login', '/team-join'].includes(location.pathname);
  ...
  return (
    <div className="App">
      ...
      <Routes>
        ...
        <Route 
          path="/login" 
          element={!user ? <Login setUser={setUser} /> : <Navigate to={currentTeam ? "/dashboard" : "/team-join"} />} 
        />

        {/* Team Selection Route */}
        <Route 
          path="/team-join" 
          element={user ? <TeamJoin onTeamSelected={handleTeamSelected} /> : <Navigate to="/login" />} 
        />

        {/* Protected Routes (require team) */}
        <Route 
          path="/dashboard" 
          element={user && currentTeam ? (
            <>
              <TournamentSetup />
              <GameRecords />
            </>
          ) : user ? <Navigate to="/team-join" /> : <Navigate to="/login" />} 
        />

        <Route 
          path="/create-game" 
          element={user && currentTeam ? <CreateTournament /> : (user ? <Navigate to="/team-join" /> : <Navigate to="/login" />)} 
        />

        <Route 
          path="/tournaments" 
          element={user && currentTeam ? <TournamentsList /> : (user ? <Navigate to="/team-join" /> : <Navigate to="/login" />)} 
        />

        <Route 
          path="/tournaments/:tournamentId/matches" 
          element={user && currentTeam ? <TournamentMatches /> : (user ? <Navigate to="/team-join" /> : <Navigate to="/login" />)} 
        />

        <Route 
          path="/leaderboard/:tournamentId" 
          element={user && currentTeam ? <Leaderboard /> : (user ? <Navigate to="/team-join" /> : <Navigate to="/login" />)} 
        />

        <Route 
          path="/players" 
          element={user && currentTeam ? <RegisteredPlayers /> : (user ? <Navigate to="/team-join" /> : <Navigate to="/login" />)} 
        />

        <Route 
          path="/team-admin" 
          element={user ? <TeamAdmin /> : <Navigate to="/login" />} 
        />
